---
url: /orange-orm-docs/updating-rows.md
---
# Updating rows
...
In our illustration, the `order` table owns both the `deliveryAddress` and the `lines` tables because they're part of a hasOne/hasMany relationship.

Contrastingly, the `customer`is part of a reference relationship and thus can't be updated here. But you can detach the reference to the customer by assigning it to null or undefined.\
*(Setting `order.customerId` to null or undefined achieves the same result.)*
