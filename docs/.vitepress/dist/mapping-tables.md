---
url: /orange-orm-docs/mapping-tables.md
---
# Mapping tables
...
Conversely, the `customer` table is independent and can exist without any knowledge of the `order` table. Therefore we say that the `order` table references the `customer` table - necessitating the existence of a `customerId` column in the `order` table.
